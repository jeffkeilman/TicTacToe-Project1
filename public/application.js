...
var _onLoginRegister = function _onLoginRegister(event) {
  ...
  if ($('#isRegister').is(':checked')) {
    ...
    if (data.credentials.password === data.credentials.password_confirmation) {
      api.register(data).then(ui.registerSuccess).catch(ui.registerFailure);
    } else {
      ui.passwordMismatch();
    }
  } else {
    ...
    api.login(data).then(ui.signOnSuccess).then(_loadGames).catch(ui.signOnFailure);
  }
};

var _loadGames = function _loadGames() {
  api.loadGames().then(ui.loadGames).catch(ui.loadGamesFailure);
};

var _loadGame = function _loadGame() {
  if (+$('#selectGameSave').text()) {
    var id = $('#selectGameSave').text();
    api.loadGame(id).then(ui.displayGame).catch(ui.loadGameFailure);
  } else {
    ui.noGameToLoad();
  }
};

var _onLogout = function _onLogout(event) {
  event.preventDefault();

  api.logout().then(ui.logoutSuccess).catch(ui.logoutFailure);
};

var _onChangePass = function _onChangePass(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
...
var _onNewGame = function _onNewGame(event) {
  event.preventDefault();
  ...
  api.newGame().then(ui.displayGame).catch(ui.newGameFailure);
};

var _showHidePassConf = function _showHidePassConf() {
  if ($('#isRegister').is(':checked')) {
...
var _handleClickedCell = function _handleClickedCell(cell) {
  var over = false;
  var givenIndex = Object.keys(ui.gameMap).find(function (key) {
    return ui.gameMap[key] === $(cell).prop('id');
  });
  var length = 0;
  $(cell).prop('disabled', true);
  for (var x = 0; x < store.user.currentGame.cells.length; x++) {
    if (store.user.currentGame.cells[x]) {
      length++;
    }
  }
  var xOrO = _whosTurn(length);
  ...
  api.updateGame(data).catch(ui.updateFailure);
...
var _whosTurn = function _whosTurn(turns) {
  return turns % 2 === 0 ? 'X' : 'O';
};

var _checkForWin = function _checkForWin() {
  var game = store.user.currentGame.cells;
...
var _goBack = function _goBack(event) {
  event.preventDefault();
  _loadGames();
  ui.goBack();
};

var _onClickHist = function _onClickHist(event) {
  event.preventDefault();
  api.loadFinishedGames().then(ui.showFinishedGames).catch(ui.loadFinishedGamesError);
};

var _onClickStats = function _onClickStats(event) {
  event.preventDefault();
  api.loadFinishedGames().then(ui.showStats).catch(ui.showStatsError);
};
...
var addEventHandlers = function addEventHandlers() {
  ...
  $('#gameSelectionArea').on('click', '.drop-down-items', function (event) {
    $('#selectGameSave').html($(event.currentTarget).text() + ' <span class="caret" id="hack"></span>');
  });
...
var loadFinishedGames = function loadFinishedGames() {
  return $.ajax({
...
var loadGame = function loadGame(id) {
  return $.ajax({
...
module.exports = {
  ...
  loadFinishedGames: loadFinishedGames
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var passwordMismatch = function passwordMismatch() {
...
var signOnSuccess = function signOnSuccess(data) {
...
var signOnFailure = function signOnFailure() {
  $('#displayFeedback').show();
  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Hmm... looks like we don\'t have an account with that email and password.<div>');
  _clearLoginModal();
};

var registerSuccess = function registerSuccess() {
  $('#displayFeedback').show();
  $('#displayFeedback').html('<div class="alert alert-success" role="alert">You are all set! Login to play!</div>');
  _clearLoginModal(true);
};

var registerFailure = function registerFailure() {
  $('#displayFeedback').show();
  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Hmm... looks like we might already have an account with this information.<div>');
  _clearLoginModal(true);
};
...
var logoutFailure = function logoutFailure() {
  ...
  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">There is something wrong with the logout. Harass the developer!<div>');
};

var changePassSuccess = function changePassSuccess() {
  $('#displayFeedback').show();

  $('#displayFeedback').html('<div class="alert alert-success" role="alert">Password changed!</div>');
...
var changePassFailure = function changePassFailure() {
  ...
  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Password change failed!</div>');
  _clearChangePassModal();
};
...
var newGameFailure = function newGameFailure() {
  ...
  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">New game creation failed!</div>');
};

var loadGames = function loadGames(data) {
  $('#gameSelectionArea ul').empty();
  data.games.forEach(function (game) {
    $('#gameSelectionArea ul').append('<li><a class="drop-down-items" href="#">' + game.id + '</a></li>');
  });
};

var loadGamesFailure = function loadGamesFailure() {
  $('#displayFeedback').show();

  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Failed to load your games!</div>');
};

var displayGame = function displayGame(data) {
  $('#histStatsRow').hide();
  $('#displayFeedback').hide();
  var game = data.game['cells'];
  // clean game screen
  ...
  for (var _x = 0; _x < game.length; _x++) {
    if (game[_x]) {
      $('#' + gameMap[_x]).text(game[_x]);
      $('#' + gameMap[_x]).prop('disabled', true);
      turns++;
    }
  }
...
var loadGameFailure = function loadGameFailure() {
  $('#displayFeedback').show();

  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Failed to load your game!</div>');
};

var noGameToLoad = function noGameToLoad() {
  $('#displayFeedback').show();

  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Please choose a game before pressing that button.</div>');
};

var markIt = function markIt(cell) {
  var length = 0;
  for (var x = 0; x < store.user.currentGame.cells.length; x++) {
    if (store.user.currentGame.cells[x]) {
      length++;
...
var updateFailure = function updateFailure() {
  ...
  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Unable to save your game!</div>');
};
...
var showFinishedGames = function showFinishedGames(data) {
  $('#histStatsRow ol').empty();
  var games = data.games;

  // build games list
  var gamesLists = _winningLosingTiesLists(games);

  for (var x = 0; x < gamesLists.winners.length; x++) {
    $('#histStatsRow ol').append('<li>' + gamesLists.winners[x].id + ': WIN!</li>');
  }

  for (var _x2 = 0; _x2 < gamesLists.losers.length; _x2++) {
    $('#histStatsRow ol').append('<li>' + gamesLists.winners[_x2].id + ': LOSS!</li>');
  }

  for (var _x3 = 0; _x3 < gamesLists.losers.length; _x3++) {
    $('#histStatsRow ol').append('<li>' + gamesLists.ties[_x3].id + ': TIE!</li>');
  }
...
var showStatsError = function showStatsError(data) {
  $('#displayFeedback').show();

  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Unable to load stats!</div>');
};

var loadFinishedGamesError = function loadFinishedGamesError() {
  $('#displayFeedback').show();

  $('#displayFeedback').html('<div class="alert alert-danger" role="alert">Unable to load game history!</div>');
};

var _lockBoard = function _lockBoard() {
  for (var key in gameMap) {
    $('#' + gameMap[key]).prop('disabled', true);
  }
};

var _winningLosingTiesLists = function _winningLosingTiesLists(games) {
  var winningList = [];
  var losingList = [];
  var tiesList = [];
  for (var x = 0; x < games.length; x++) {
    var winner = _checkForWinner(games[x].cells);

    if (winner) {
      var playerExists = games[x]['player_' + winner.toLowerCase()];
...
var _checkForWinner = function _checkForWinner(game) {
  if (game[0] && game[3] && game[6]) {
    if (game[0] === game[3] && game[3] === game[6]) {
      return game[0];
    }
...
var _resetSaveGameMessage = function _resetSaveGameMessage() {
  $('#selectGameSave').html('Saved Games <span class="caret" id="hack"></span>');
};

var _setTurnMessage = function _setTurnMessage(turns) {
...
var _clearChangePassModal = function _clearChangePassModal() {
  $('#changePassModal').modal('hide');
  $('#oldPassLabel').val('');
  $('#newPassLabel').val('');
};

var _clearLoginModal = function _clearLoginModal(all) {
...
var _prepSelectionArea = function _prepSelectionArea() {
  $('#loginButton').hide();
  $('#displayFeedback').hide();
  $('#changePass').show();
  $('#logoutButton').show();
...
var _restoreMain = function _restoreMain() {
  _resetSaveGameMessage();
  $('#histStatsRow').hide();
  $('#gameArea').hide();
  $('#logoutButton').hide();
...
module.exports = {
  ...
  loadGames: loadGames,
  loadGamesFailure: loadGamesFailure,
  displayGame: displayGame,
  loadGameFailure: loadGameFailure,
  markIt: markIt,
  gameMap: gameMap,
  handleWin: handleWin,
  ...
  loadFinishedGamesError: loadFinishedGamesError,
  showStats: showStats,
  showStatsError: showStatsError,
  handleDraw: handleDraw,
  passwordMismatch: passwordMismatch,
  noGameToLoad: noGameToLoad
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
